    ...
    endpoints: [
      {
        ...
        description:
          'Unauthenticated public feed endpoint. When the immediate client IP is a configured trusted proxy, X-Real-IP, X-Forwarded-For, X-Forwarded-Proto, and X-Forwarded-Host are honoured for client IP and feed URL generation; otherwise forwarded headers are ignored.',
        public: true,
        params: [
          {
            name: 'token',
            in: 'path',
            type: 'string',
            desc: '32-character hexadecimal subscription token.',
        ...
        errorResponses: [
          ...
          },
          {
            status: 403,
            contentType: 'text/plain',
            description: 'Request blocked by user-agent policy or IP limit.',
          },
          {
            status: 404,
            contentType: 'text/plain',
            description:
              'Subscription token is invalid, missing, disabled, or hidden by user-agent policy.',
          },
          {
            status: 500,
            contentType: 'text/plain',
      ...
      },
      {
        ...
        description:
          'Unauthenticated public provider endpoint. It checks the same token, user-agent, and trusted-proxy header rules as /feed/:token, but does not bind a new IP slot.',
        public: true,
        params: [
          {
            name: 'token',
        ...
        errorResponses: [
          ...
          },
          {
            ...
            contentType: 'text/plain',
            description:
              'Subscription lookup, IP check, rendering, or user-agent policy check failed.',
          },
        ],
      },
      {
        ...
        summary:
          'List inbound picker options for the converter, including export capability, CDN TLS capability, and enabled client traffic metadata.',
      },
      {
        method: 'GET',
        path: '/panel/api/subconverter/list',
        summary:
          'List every subscription-conversion entry with linked inbounds, usage stats, and bound IP count.',
      },
      {
        method: 'GET',
        path: '/panel/api/subconverter/get/:id',
        summary:
          'Fetch one subscription-conversion detail by ID, including top-level entry fields, bound IP records, and the most recent access logs.',
      ...
      },
      {
        ...
        summary:
          'Delete one subscription-conversion entry and its related bindings, logs, stats, and inbound links.',
        params: [{ name: 'id', in: 'path', type: 'number', desc: 'Subscription-conversion ID.' }],
      },
      {
        method: 'GET',
        ...
        summary:
          'Read global subconverter settings, including User-Agent filtering keywords and reject status.',
      },
      {
        method: 'POST',
        path: '/panel/api/subconverter/settings',
      ...
      },
      {
        ...
        summary:
          'Return recent known-token public feed access logs across subscription-conversion entries.',
        params: [
          {
            name: 'limit',
            in: 'query',
            type: 'number',
            desc: 'Maximum number of logs to return. Defaults to 100 and is capped at 500.',
            optional: true,
            defaultValue: 100,
            maximum: 500,
          },
        ...
        params: [
          {
            ...
            in: 'path',
            type: 'number',
            desc: 'Subscription-conversion ID.',
          },
